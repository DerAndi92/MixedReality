...
public class RescueTargetCollider : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.tag == "RescueTarget")
        {
            ...
            else if (GameController.Instance.isHelicopterTargetTracked && other.name == "TargetHeliLandingPlace")
            {
                GameObject test = GameObject.Find("HeliPosition");
                GameController.Instance.heliPosition = test;
                GameController.Instance.eventHelicopter = true;
            }
            else if (GameController.Instance.isPoliceTargetTracked && other.name == "TargetPoliceDepartment")
            {
